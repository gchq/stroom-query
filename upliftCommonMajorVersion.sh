...
for versionedDir in `find ${SRC_DIR} -type d -name "*${oldMajorVersion}"`; do 
...
echo "About to change `find ${SRC_DIR} -type f -name "*.java" | xargs grep "${PACKAGE_PREFIX_QUERY_API}.${oldMajorVersion}" | wc -l` lines"
find ${SRC_DIR} -type f -name "*.java" | xargs sed -i "s/${PACKAGE_PREFIX_QUERY_API}\.${oldMajorVersion}/${PACKAGE_PREFIX_QUERY_API}.${newMajorVersion}/g"

echo ""
echo "Re-versioning ${PACKAGE_PREFIX_DATASOURCE_API} packages"
echo "About to change `find ${SRC_DIR} -type f -name "*.java" | xargs grep "${PACKAGE_PREFIX_DATASOURCE_API}.${oldMajorVersion}" | wc -l` lines"
find ${SRC_DIR} -type f -name "*.java" | xargs sed -i "s/${PACKAGE_PREFIX_DATASOURCE_API}\.${oldMajorVersion}/${PACKAGE_PREFIX_DATASOURCE_API}.${newMajorVersion}/g"
